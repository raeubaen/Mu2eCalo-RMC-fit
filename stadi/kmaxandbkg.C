// sbloccando alfa non va malissimo
...
double ethmin = 60, ethmax = 150; // range of photon energy
...
double efitmin = 70, efitmax = 120; // fit range
...
void kmaxandbkg(){
  ...
  double par[3] = {kmax, alpha, bkg};
  eexp_hist = GetHist(par);

  double lim[3][2] = {{89, 91}, {1.9999, 2.0001}, {0.004, 0.005}};
  double nbins = 40;
  double w[3];
  for(int i=0; i<3; i++) w[i] = (lim[i][1]-lim[i][0])/nbins;
  ...
  */
  ...
  ROOT::Math::Functor f(&ChiSquare, 3);
  minimum->SetFunction(f);

  double step[3] = {0.1, 0.01, 0.0003}; // very important to tune if the minimization fails - 0.005 is a good value
  double variable[3] = { kmax, alpha, bkg};    // starting point

  // Set the free variables to be minimized !
  minimum->SetVariable(0,"kmax",variable[0], step[0]);
  minimum->SetVariable(1,"alpha",variable[1], step[1]);
  minimum->SetVariable(2,"bkg",variable[2], step[2]);
  for(int i=0; i<3; i++) minimum->SetVariableLimits(i, lim[i][0], lim[i][1]);
  ...
  g1->Draw();
  minimum->Scan(2, nstep, x, y);
  TGraph *g2 = new TGraphErrors(100, x, y);
  TCanvas *c8 = new TCanvas("c8", "c");
  g2->Draw();

  double xx[100], yy[100];
  TCanvas *c9 = new TCanvas("c9", "contour");
  minimum->Contour(0, 2, nstep, xx, yy);
  TGraph *g3 = new TGraph(100, xx, yy);
  TGraph *star = new TGraph(1, (double[]){kmax}, (double[]){bkg});
  ...
  cout << "bkg pull: " << (xs[2] - bkg)/errs[2] << endl;

  cout << "Correlation Coefficient between kmax and alpha: " << minimum->Correlation(0, 1) << endl;
  cout << "Correlation Coefficient between kmax and bkg: " << minimum->Correlation(0, 2) << endl;
  cout << "Correlation Coefficient between bkg and alpha: " << minimum->Correlation(2, 1) << endl;
  ...
  for(int i=0; i<nbins; i++){
    for(int j=0; j<nbins; j++){
      chi2_clone->SetBinContent(i, j, chi2_clone->GetBinContent(i, j) - loss_offset);
    }
  }
  ...
  TString outFileName = "outkmaxbkg.root";
