...
double flmin = -30, flmax=20; // range of legit fluctuactions
int nflbins = (int)((flmax-flmin)/binw);

double ethmin = 0, ethmax = 150; // range of photon energy
int ethbins = (int)((ethmax-ethmin)/binw);
double erecomin = 50, erecomax = 95; // range of reconstructed energy
...
void basic_confitReso(){
  ...
  double lim[2][2] = {{89.9, 90.1}, {1.98, 2.02}};
  double nbins = 40;
  double w[2];
  ...
  for(int i=0; i<nbins; i++){
    for(int j=0; j<nbins; j++){
      chi2_clone->SetBinContent(i, j, chi2_clone->GetBinContent(i, j) - chi2min);
    }
  }
