...
double ethmin = 50, ethmax = 90; // range of photon energy
...
double ChiSquare(const double *par){
   ...
   for(int i=0; i<erecobins; i++){
      ...
      if(err_sq != 0) sum += (y1 - y2) * (y1-y2) / err_sq;
   }
...
// main function
void clappSmearingAndFitting(){


  double kmax = 90.2, alpha = 2.1;

  TFile *resoFile = new TFile("Reso.root");

  cout << "Evaluating smearing matrix" << endl;
  // smearing matrix
  TH2F *smearing_pdf = new TH2F("smearing_pdf", "Smearing PDF", ethbins, ethmin, ethmax, erecobins, erecomin, erecomax);
  for(int i=0; i<ethbins; i++){
  ...
  f_eth = new TF1("f_eth", "(1-2*x/[0]+2*x/[0]*x/[0])*x/[0]*(1-x/[0])^[1]", ethmin, ethmax);
  ...
  minimum->SetTolerance(1); // very important to tune if the minimization fails - 0.01 is a good value
  ...
  double step[2] = {0.005,0.001}; // very important to tune if the minimization fails - 0.05 is a good value
  ...
  f_eth_clone->SetParameters(kmax, alpha);
  f_eth_clone->Draw("same");
  TLegend *ll = new TLegend();
  ll->AddEntry(f_eth, "After fit");
  ll->AddEntry(f_eth_clone, "Before fit");
  ll->Draw();

  TH1F *ereco_hist = GetErecoHist(xs);
  ereco_hist->Scale(1e6);
  ...
  TString outFileName = Form("out_fitstartat%iMeV.root", (int)efitmin);
